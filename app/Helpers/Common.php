...
if (! function_exists('saveUserConfig')) {
    function saveUserConfig($name, $value='', $expectsJson=false){ // necesito el & de pasar por referencia para que no muera si la variable no está seteada
...
if (! function_exists('getUserConfig')) {
    function getUserConfig($name, $default=null){ // necesito el & de pasar por referencia para que no muera si la variable no está seteada
...
// usada en los print de los componentes para imprimir campos que pudieran tener saltos de linea y necesitenb el br y en caso de no existir, mandar default
if (! function_exists('justForGods')) {
    function justForGods($user=null){ // necesito el & de pasar por referencia para que no muera si la variable no está seteada
        ...
        if($user){
            if(!$user->isGod()){
                abort(403,"Como " . $user->name . " no tenés los permisos necesarios para lo que intentás hacer.");
            }
            else{
                return true;
            }
